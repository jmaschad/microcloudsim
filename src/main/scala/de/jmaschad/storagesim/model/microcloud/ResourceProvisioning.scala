...
class ResourceProvisioning(storageSystem: StorageSystem, networkBandwidth: Double, cloud: MicroCloud) {
    ...
    class NetUpProvisioner extends Provisioner {
        def update(timeElapsed: Double, jobs: Seq[Job]) = jobs.foreach(_.progressNetUp(timeElapsed * uploadBandwidthPerJob))
        def expectedCompletions(jobs: Seq[Job]): Array[Double] = jobs.map(j => (j.netUpSize.getOrElse(0.0) / uploadBandwidthPerJob).max(0)).toArray
    }

    class NetDownProvisioner extends Provisioner {
        def update(timeElapsed: Double, jobs: Seq[Job]) = jobs.foreach(_.progressNetDown(timeElapsed * downloadBandwidthPerJob))
        def expectedCompletions(jobs: Seq[Job]): Array[Double] = jobs.map(j => (j.netDownSize.getOrElse(0.0) / downloadBandwidthPerJob).max(0)).toArray
    }
